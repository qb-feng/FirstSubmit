...
public partial class Logger : Singleton<Logger>
{
 ...
 Application.dataPath + "/../";
...
#if SYNC_LOG
            LogDelegate.RegisterLogCallback(LogCallback);
#else
    ...
    static void LogCallback(string condition, string stackTrace, LogType type)
    {
        ...
        //return;
        Logger.Instance.Log(type, "condition:{0}, stackTrace:{1}", condition, stackTrace);
    }

    static void LogInfo(LogType level, string format, params object[] objs)
    {
        if (Logger.IsCreate == false)
            return;

        Logger.Instance.Log(level, format, objs);
    }
    ...
    public static void MemoryLog(string format, params object[] objs)
    {
        LogInfo(LogType.Log, format, objs);
    }
    ...
    object m_write_lock = new object();
    // 锁
