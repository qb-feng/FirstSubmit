...
//            else
        ...
        /// <returns></returns>
        public AssetBundle LoadSN(string shortName)
        {
            string bundleName = GetAssetBundleFullName(shortName.ToLower());
            return LoadBN(bundleName);
        }

        public AssetBundle LoadFN(string fullName)
        {
            string bundleName = HashUtil.Get(fullName.ToLower()) + ".ab";
            return LoadBN(bundleName);
        }

        public AssetBundle LoadBN(string bundleName)
        {
#if _AB_MODE_
            string cachedFile = string.Format("{0}/{1}", pathResolver.BundleCacheDir, bundleName);
            string sourceFile = pathResolver.GetBundleSourceFile(bundleName, false);
            if (File.Exists(cachedFile))
            {
                return AssetBundle.LoadFromFile(cachedFile);
            }
            else if(_depInfoReader == null)
            {
                return AssetBundle.LoadFromFile(sourceFile);
            }
            else if(_depInfoReader != null && _depInfoReader.infoMap.ContainsKey(bundleName))
            {
                return AssetBundle.LoadFromFile(sourceFile);
            }
            else    
            {
                Debug.LogWarning("Load AB Is Null : " + bundleName);
                return null;
            }
#else
            return null;       
