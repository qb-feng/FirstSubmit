...
public class SharkMovingLayer : MonoBehaviour
{
    ...
    void Update()
    {
        if (!paused)
        {
            ...
            foreach (Transform item in activeElements)
            {
                ...
                item.parent.transform.position -= Vector3.right * startingSpeed * speedMultiplier * Time.deltaTime;
    ...
    public virtual void SpawnElement(bool inMiddle)
    {
        ...
        int random = Random.Range(0, inactive.Count);
        if (inactive.Count <= random)
        {
            Debug.Log("random : " + random + "inactive.Count : " + inactive.Count + "Full Path : " + transform.GetFullHierarchy());
            return;
        }
        Transform item = inactive[random];
