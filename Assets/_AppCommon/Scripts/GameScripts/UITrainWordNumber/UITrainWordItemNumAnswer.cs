...
public class UITrainWordItemNumAnswer : UIBaseInit
{
    ...
    private string m_downTrainBoxPrefix = "DownTrainBox";
    ...
    public void Init(int colorIndex, float tweenDelay, int currentIndex)
    {
        ...
        for (int i = 1; i < 4; i++)
        {
            ...
            Get<Image>(m_downTrainBoxPrefix + i).sprite = GetS(boxPrefix + 3);
        }
    ...
    public void DownTrainBoxActive(bool active)
    {
        for (int i = 0; i < 3; i++)
        {
            Get(m_downTrainBoxPrefix + (i + 1)).SetActive(active);
        }
    }
    ...
    public Tweener TweenCome(float delay)
    {
        ...
        Train.anchoredPosition = new Vector2(1700, y);
    ...
    {
        Tweener tween = Train.DOAnchorPosX(-1700, 2f);
    ...
    public void SetTrainWord(string sentence, string positive)
    {
        ...
        for (int i = 0; i < words.Length; i++)
        {
            ...
            Get<TextMeshProUGUI>(m_downTrainWordPrefix + (i + 1)).text = words[i];
        }
    }
    ...
    public void MatchAdd()
    {
        ...
        if (m_matchTrainBoxCount == 3)
        {
            ...
            s_matchTrainCount++;
            if (UITrainWordItemNumAsk.s_matchTrainCount == 1 && s_matchTrainCount == 1)
            {
