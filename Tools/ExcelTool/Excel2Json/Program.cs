...
namespace excel2json
{
    ...
    sealed partial class Program
    {
        ...
        private static void Run(Options options)
        {
            ...
            var paths = Directory.GetFiles(excelPath, "*.xlsx");
            foreach (var path in paths)
            {
                ...
                using (FileStream excelFile = File.Open(path, FileMode.Open, FileAccess.Read))
                {
                    ...
                    for (int i = 0; i < book.Tables.Count; i++)
                    {
                        ...
                        string jsonName = sheet.TableName;
                        ...
                        if (options.JsonPath != null && options.JsonPath.Length > 0)
                        {
                            ...
                            exporter.SaveToFile(options.JsonPath + jsonName + ".json", cd);
                            //exporter.SaveToFile(sheet.TableName + ".json", cd);
                        }
                        ...
                        {
                            string excelName = Path.GetFileName(path);
